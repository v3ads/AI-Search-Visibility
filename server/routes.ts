...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.post("/api/suggest-competitors", requireAuth, async (req, res) => {
    try {
      ...
      const { getSiteCompetitors } = await import("./site-intelligence");

      const competitors = await getSiteCompetitors(
        String(domain).slice(0, 100),
        String(brandName).slice(0, 80),
        String(industry || "general business").slice(0, 80)
      );
